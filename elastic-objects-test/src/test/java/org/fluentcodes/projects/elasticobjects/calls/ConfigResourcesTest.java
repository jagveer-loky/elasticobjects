...
public class ConfigResourcesTest {
    ...
    public void givenModel_whenCreate_thenThrowsException()  {
        final ModelConfig model = ProviderRootTestScope
                .EO_CONFIGS
                .findModel(ResourceProperties.class);
        Assert.assertEquals(ResourceProperties.class, model.getModelClass());
        Assertions
                .assertThatThrownBy(
                        () -> { model.create();})
                .isInstanceOf(EoException.class)
                .hasMessageContaining("Could not create empty instance from model for 'ResourceProperties'");
