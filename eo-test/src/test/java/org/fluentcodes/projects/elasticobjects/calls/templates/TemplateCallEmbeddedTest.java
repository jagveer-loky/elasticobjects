...
import org.fluentcodes.projects.elasticobjects.test.TestDataProvider;
...
public class TemplateCallEmbeddedTest {
    ...
    public void callEmbedded0()  {
        ...
        final TemplateCall action = new TemplateCall().setConfigKey( T_EMBEDDED_0);
    ...
    public void callEmbedded0WithPlaceHolder()  {
        final EO adapter = TestDataProvider.createAdapterEmbedded();
        final TemplateCall action = new TemplateCall().setConfigKey(T_EMBEDDED_0_WITH_PLACE_HOLDER);
        final String result = action.execute(adapter);
        Assert.assertNotNull(result);
        //AssertEO.compare(result);
    }
    ...
    public void callEmbedded2()  {
        final EO adapter = TestDataProvider.createAdapterEmbedded();
        final TemplateCall action = new TemplateCall().setConfigKey( T_EMBEDDED_2);
        final String result = action.execute(adapter);
        Assert.assertNotNull(result);
        //AssertEO.compare(result);
    }
    ...
    public void callEmbedded2WithPlaceHolder()  {
        final EO adapter = TestDataProvider.createAdapterEmbedded();
        final TemplateCall action = new TemplateCall().setConfigKey( T_EMBEDDED_2_WITH_PLACE_HOLDER);
        final String result = action.execute(adapter);
        Assert.assertNotNull(result);
        //AssertEO.compare(result);
    }
