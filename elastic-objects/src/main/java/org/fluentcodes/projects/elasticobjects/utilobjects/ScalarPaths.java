...
//        }
    ...
    /**
     * Returns the paths matching with ^path and Path.DELIMITER
     *
     * @param path
     * @return
     */
    public ScalarPaths getSubPaths(String path) {
        String pathStart = path + Path.DELIMITER;
        String replacePattern = "^" + path + Path.DELIMITER;
        ScalarPaths result = new ScalarPaths();
        for (String key : this.names) {
            if (key.startsWith(pathStart)) {
                String subPath = key.replaceAll(replacePattern, "");
                result.put(subPath, this.get(key));
            }
        }
        return result;
    }
    ...
    public void put(String myPath, Object value) {
        ...
        if (value instanceof ScalarPaths) {
            ...
            for (int i = 0; i < names.size(); i++) {
                ...
                Path subPath = pathObject.addPath(name);
...
//            return;
    ...
    public ScalarPaths diff(ScalarPaths compare) {
        ScalarPaths diff = new ScalarPaths();
        List<String> comparedPaths = new ArrayList<String>();
        for (String path : this.names) {
            comparedPaths.add(path);
            Object thisValue = this.get(path);
            Object compareValue = compare.get(path);
            if (thisValue == null && compareValue == null) {
                continue;
            }
            if (compareValue == null) {
                diff.put(path, new Object[]{thisValue, compareValue});
                continue;
            }
            try {
                if (!ScalarComparator.compare(thisValue, compareValue)) {
                    diff.put(path, new Object[]{thisValue, compareValue});
                }
            } catch (Exception e) {
                e.printStackTrace();
        ...
        for (String path : compare.paths()) {
            if (comparedPaths.contains(path)) {
                continue;
            }
            Object thisValue = this.get(path);
            Object compareValue = compare.get(path);
